...
- **LRM 混合模型**：`Work → Release → Medium → Track → AssetFile`，纯净标题无杂质，同一录音或母版可被多版发行共同引用，彻底杜绝数据冗余。
- **多维标签 + 虚拟货架**：形态、介质、规格、主题自由组合，彻底废弃死板的 `media_type`；货架由标签规则动态生成，“宫崎骏 1990s 剧场版”“DSD256 爵士”点一下即是。
- **自适应封面与多语言**：封面支持自然宽高比展示与 `cover_aspect` 手动精细控制（1:1 / 2:3 / 3:4）；基于 `work_translations` 构建完整本地化回退链。
...
- **高速 S3 兼容对象存储**：全面采用 RustFS 替代传统方案，兼具极致性能与安全直传。
- **毫秒级全域搜索**：基于 OpenSearch 2.x 构建分词与 Facet 聚合检索，支持断网降级与纯净多语言检索。
...
```bash
cp .env.example .env
# 用编辑器打开 .env，填入强随机密钥配置：
# DB_PASSWORD / MINIO_ROOT_PASSWORD (RustFS) / JWT_SECRET

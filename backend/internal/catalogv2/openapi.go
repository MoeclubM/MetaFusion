...
func OpenAPI() map[string]any {
	...
	for _, r := range [][6]string{
		...
		{"/auth/login", "post", "Sign in; return token and HttpOnly cookie", "Credentials", "Result", ""}, {"/auth/me", "get", "Current account", "", "User", "auth"}, {"/auth/logout", "post", "Revoke current session", "", "Result", "auth"}, {"/admin/users", "post", "Create editor (administrator only)", "Credentials", "User", "auth"},

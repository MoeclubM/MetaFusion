...
func OpenAPI() map[string]any {
	...
	for _, r := range [][6]string{
		...
		{"/importer/preview", "post", "Preview external catalog entry via an outbound fetch (requires catalog.import.submit, 10/min per IP)", "ImporterPreviewRequest", "ImporterPreviewResponse", "auth"},
		{"/importer/import", "post", "Import previewed entry with evidence (requires catalog.import.submit)", "ImporterImportRequest", "ImporterImportResponse", "auth"},

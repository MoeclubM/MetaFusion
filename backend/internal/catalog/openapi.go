...
func OpenAPI() map[string]any {
	...
	for _, r := range [][6]string{
		...
		{"/importer/import", "post", "Import previewed entry with evidence (requires catalog.import.submit; download_cover=false skips remote cover refs; is_master_verified and media_type_hint are rejected; has_release=true requires mediums; link_mode merge_translations is rejected)", "ImporterImportRequest", "ImporterImportResponse", "auth"},

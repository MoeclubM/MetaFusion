...
func OpenAPI() map[string]any {
	...
	for _, p := range []struct{ name, desc string }{
		...
		{"q", "Substring match on the base title and on the whole translations object. The value is validated before it reaches SQL: invalid UTF-8, control characters (except TAB) and values over 256 bytes are 400 invalid_query_param / query_too_long, and a raw query string the URL parser cannot decode (a bare %) is 400 as well instead of being dropped by net/url"},
	} {

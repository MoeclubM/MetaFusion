...
func OpenAPI() map[string]any {
	...
	for _, r := range [][6]string{
		...
		{"/catalog/definitions", "get", "Published dynamic definitions", "", "DefinitionVersion", ""}, {"/catalog/entities", "get", "Entity search (kind/kinds/q/type/types/status/work_id/content_unit_id/release_id/medium_id/parent_id/field/value/tags; items + real COUNT total; 120/min per IP)", "", "Result", ""}, {"/catalog/entities", "post", "Create entity with evidence (supports Idempotency-Key, 24h)", "Edit", "Entity", "auth"},
	...
	params := []any{}
	for _, name := range []string{"kind", "kinds", "type", "types", "status", "q", "field", "value", "work_id", "content_unit_id", "release_id", "medium_id", "parent_id", "tags", "limit", "offset"} {

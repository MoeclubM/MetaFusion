...
func OpenAPI() map[string]any {
	...
	for _, v := range []any{Entity{}, EntityLinksPage{}, ReleaseTOC{}, Edit{}, Relation{}, RelationEdit{}, LifecycleEdit{}, UnpublishEdit{}, DefinitionVersion{}, DefinitionVersionItem{}, DefinitionRollback{}, DefinitionDiff{}, Definitions{}, ExternalDatabase{}, Shelf{}, HomePreferences{}, HomeSection{}, UserContributions{}, ContributionItem{}, UserContributionStats{}, ImporterPreviewRequest{}, ImporterPreviewResponse{}, ImporterImportRequest{}, ImporterImportResponse{}, ImporterSource{}, VersionInfo{}, DefinitionImpact{}, Notification{}} {
	...
	for _, r := range [][6]string{
		...
		{"/catalog/definitions", "get", "Published dynamic definitions, fixed entity-skeleton names, and a read-only registry of structural and semantic relationship rules", "", "PublishedDefinitions", ""}, {"/catalog/developer/request-logs", "get", "Own API request log, newest first (authenticated calls only; route is the gin template path without entity ids or query strings; 30-day retention)", "", "Result", ""},
		{"/catalog/entities", "get", "Entity search (kind/kinds/q/type/types/status/work_id/content_unit_id/release_id/medium_id/parent_id/field/value/tags/original_language/has_pictures; field takes a published field code; a dotted path works only when every level is searchable and enabled, so the seed definitions expose top-level codes such as tags, duration, edition_date and barcode; items + real COUNT total, plus 400 invalid_sort / invalid_order for a sort key or direction outside the whitelist, and 400 invalid_query_param / query_too_long / invalid_limit / invalid_offset / invalid_page / pagination_conflict for parameters that cannot be sent as given (see each parameter). Rate limited to 120 requests per minute per IP: every response from a limited route carries X-RateLimit-Limit, X-RateLimit-Remaining and X-RateLimit-Reset (seconds until the window resets), and an exhausted budget answers 429 rate_limited with Retry-After)", "", "Result", ""}, {"/catalog/entities", "post", "Create entity with evidence (supports Idempotency-Key, 24h)", "Edit", "Entity", "auth"},
		...
		{"/catalog/releases/{id}/toc", "get", "Read a visible release with ordered media and tracks plus deduplicated visible expressions in one consistent snapshot (120/min per IP). Hidden child entities and their inclusions are omitted; each entity carries its own version and definition_version identifies the published field contract. Errors: 400 invalid_id / not_release, 404 not_found", "", "ReleaseTOC", ""},
		{"/catalog/entities/{id}", "get", "Read visible entity", "", "Entity", ""}, {"/catalog/entities/{id}", "put", "Replace entity with optimistic version check. Whole-entity replacement, pictures included: the pictures array is ordered and **array order is the display order**, so pictures[0] is the cover (the server never re-sorts and taken_at is metadata, not a sort key). Optional per-picture keys: role must be an enabled code of the picture_role vocabulary (empty = undeclared, which is what all legacy pictures are), asset_id must be a storage-service asset UUID (self-hosted cover bound with binding_role=cover_image). Writes are refused with duplicate_picture (same URL twice on one entity), too_many_pictures (over 40, because every picture is copied into each revision snapshot) and invalid_term (or unknown_vocabulary when the picture_role vocabulary itself is gone) for a role code outside the vocabulary", "Edit", "Entity", "auth"}, {"/catalog/entities/{id}/resolve", "get", "Resolve merged identity", "", "Entity", ""}, {"/catalog/entities/{id}/identity", "get", "Resolve canonical identity plus historical aliases (X01 identity contract: canonical_id, aliases, entity; read-only, rewrites nothing)", "", "Result", ""}, {"/catalog/entities/identity", "post", "Batch identity resolution for cross-service alias aggregation (files/comments/favorites); JSON body {ids:[...]} up to 500, per-id failures listed in missing", "Result", "Result", ""}, {"/catalog/entities/{id}/lifecycle", "post", "Merge or retire (requires catalog.lifecycle.manage)", "LifecycleEdit", "Entity", "auth"}, {"/catalog/entities/{id}/unpublish", "post", "Demote a published entity back to draft, the only way back from published (requires catalog.lifecycle.manage; Save refuses demotion with use_lifecycle_endpoint). Request body is UnpublishEdit: expected_version, edit_note and at least one source, validated like every other write. Only published -> draft is accepted: draft/pending_review have nothing to unpublish and deleted/merged are terminal, so all four answer 400 invalid_status (never 500); a stale expected_version is 409 version_conflict. The transition is recorded in the existing revision trail as a revision row plus an entity.unpublished outbox event, so GET /catalog/entities/{id}/revisions lists it while the contribution stats keep their calibration (audit_actions still counts only entity.deleted / entity.merged)", "UnpublishEdit", "Entity", "auth"},
		...
		{"/catalog/entities/{id}/links", "get", "Read visible direct structural and semantic links from their canonical stores; each link has a direction relative to subject_id. The returned window includes endpoint entities, the currently published definitions version, and has_more. Fixed links are edited only through their owning entities. Pagination follows endpoint visibility and historical semantic attribute filtering (120/min per IP); this first version does not claim per-edge evidence or historical rule versions", "", "EntityLinksPage", ""},
		{"/catalog/compare", "get", "Compare two to six entities of any kinds (10/min per IP); releases/mediums include their containment tree", "", "Result", ""},
		...
		{"/catalog/relations", "post", "Create contextual relation (requires catalog.relation.edit; supports Idempotency-Key, persistent: same key replays the first result across restarts/replicas, same key with a different payload returns 409 idempotency_conflict)", "RelationEdit", "Relation", "auth"}, {"/catalog/relations/{id}", "put", "Replace relation context (requires catalog.relation.edit)", "RelationEdit", "Relation", "auth"}, {"/catalog/relations/{id}", "delete", "Remove relation with evidence (requires catalog.relation.edit)", "LifecycleEdit", "Result", "auth"},
		{"/admin/catalog-definitions", "get", "List definition versions (requires catalog.definitions.manage); each item carries state, created_at, created_by (when a revision row exists) and a short counts summary. include_document defaults to true and keeps the full document on every item; include_document=false omits the document key entirely (and reads no document from the database) while keeping every other metadata field, and the response-level include_document tells the client whether documents came along — read one version with GET /admin/catalog-definitions/{id}; an unparsable value is rejected with invalid_payload instead of silently returning documents", "", "DefinitionList", "auth"}, {"/admin/catalog-definitions", "post", "Save immutable draft (requires catalog.definitions.manage); non-empty definitions.structure must preserve the fixed ownership fields, target kinds, required flags, release subjects and track contents (fixed_structure_mismatch), while relation types remain editable", "DefinitionDraft", "Result", "auth"}, {"/admin/catalog-definitions/{id}/impact", "get", "Replay a definition version against all current data (requires catalog.definitions.manage). Two separate lists come back: issues blocks the publish (the definition itself is illegal, or conflicts with existing data on value semantics: unknown field, disabled field/term, wrong field type, endpoint kinds, cycles, structure rules), and dangling_references only warns — it lists existing data that points at rows which no longer exist (attributes of type entity, structural and record-level references, relation endpoints), each entry carrying scope (entity | relation), id, kind, field path, the dangling value and the reason (missing | invalid_id). Dangling references never block a publish: they are pre-existing data debt, repairing them is a data task, and letting them stop the definition would freeze new relation codes and fields on an instance forever (2026-09 incident: 31 dangling attributes.publisher values made the startup publish fail and the container crash-loop). Use mf-migrate check-refs to list the same report from the database side", "", "DefinitionImpact", "auth"}, {"/admin/catalog-definitions/{id}/publish", "post", "Publish compatible draft (requires catalog.definitions.manage); blocked only by definition issues from the impact replay, while dangling references are logged as a warning and do not stop the publish", "LifecycleEdit", "Result", "auth"},
		...
		{"/catalog/shelves/feed", "get", "Evaluate shelf rules with their items, ordered by caller preferences; each shelf's items follow its own sort key: updated (default) | created | title. For a signed-in caller the shelf list is merged before ordering: a preferences section whose slug matches a system shelf overrides that shelf for this caller only (names merged per locale, query/sort replaced, blank icon keeps the system icon), other sections are appended as caller-private shelves, order interleaves system and custom slugs, and hidden removes both kinds; every entry's shelf carries source: system (possibly overridden) or custom, so the client knows whether the row can be deleted; anonymous callers always see source: system and no sections. Each entry also returns total: the full count of matching entities for that caller (not capped by per_shelf), while items is capped for display", "", "Result", ""},
		...
		{"/notifications/internal", "post", "Service-to-service notification delivery (community -> catalog). Requires X-Internal-Token (INTERNAL_API_TOKEN); disabled with 503 internal_api_disabled when unset. The caller supplies actor_id/actor_name as the event author snapshot in the request body; no end-user bearer token is accepted or required. Validates recipient_id, type, subject and dedupe fields before writing; delivery failures are logged by the caller without failing the original community action", "NotificationDelivery", "NotificationReadResult", ""},
	} {
	...
	for _, p := range []struct{ name, desc string }{
		...
		{"limit", "Page size, default 50, max 100. A non-integer, 0, a negative value or anything above 100 is 400 invalid_limit (an empty value counts as absent) — the old behaviour silently replaced it with the default, so a caller could not tell its page size never applied"},
		{"offset", "Page offset, default 0; must be an integer >= 0, otherwise 400 invalid_offset. Mutually exclusive with page: passing both is 400 pagination_conflict rather than letting one of them quietly win"},
		{"page", "1-based page number, sugar for offset=(page-1)*limit (limit defaults to 50); must be an integer >= 1, otherwise 400 invalid_page. A page past the end answers 200 with an empty items and the real total still filled in, so the caller can tell 'nothing left' from 'nothing found'"},
		{"q", "Searches the base title and translation text, including titles, summaries and aliases. When OPENSEARCH_URL is configured and no structural/custom-field filters are present, OpenSearch supplies ranked candidate IDs; PostgreSQL still verifies substring matches, applies visibility and list filters, and reads the returned entities. Structural or custom-field filters, an unavailable or unready index, a query with no indexed hit, or more than 10,000 candidates uses the full PostgreSQL substring search. The value is validated before database search: invalid UTF-8, control characters (except TAB) and values over 256 bytes are 400 invalid_query_param / query_too_long, and a raw query string the URL parser cannot decode (a bare %) is 400 as well instead of being dropped by net/url"},
	} {
	...
	paths["/catalog/compare"].(map[string]any)["get"].(map[string]any)["parameters"] = []any{map[string]any{"name": "ids", "in": "query", "required": true, "description": "Two to six comma-separated entity UUIDs of any kinds", "schema": map[string]any{"type": "string"}}}
	...
	paths["/catalog/entities/{id}/links"].(map[string]any)["get"].(map[string]any)["parameters"] = []any{
		map[string]any{"name": "id", "in": "path", "required": true, "schema": map[string]any{"type": "string"}},
		qp("limit", "Page size, default 50, max 100; invalid values return 400", false),
		qp("offset", "Page offset, default 0; mutually exclusive with page", false),
		qp("page", "1-based page number; mutually exclusive with offset", false),
	}
	paths["/catalog/developer/request-logs"].(map[string]any)["get"].(map[string]any)["parameters"] = []any{
		qp("credential", "Filter by credential name (PAT name, exact); empty returns all", false),
		qp("limit", "Page size, default 50, max 100", false),
	}

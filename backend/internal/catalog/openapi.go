...
func OpenAPI() map[string]any {
	...
	for _, r := range [][6]string{
		...
		{"/importer/import", "post", "Import previewed entry with evidence (requires catalog.import.submit; validated with zero writes before the first save: attribute values, unknown field codes, original_language, translation rows and date fields are checked against the published definitions with the same rules as entity saves; download_cover=false skips remote cover refs; is_master_verified and media_type_hint are rejected; has_release=true requires mediums; link_mode merge_translations is rejected)", "ImporterImportRequest", "ImporterImportResponse", "auth"},

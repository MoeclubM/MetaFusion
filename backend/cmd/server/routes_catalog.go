...
func registerCatalogRoutes(
	...
	catGroup.GET("/works/:id/contents", catalogSvc.GetWorkContents)
	...
	catGroup.PUT("/mediums/:id", auth.UnifiedAuthMiddleware(cfg, db), auth.RequireEmailVerified(db), catalogSvc.UpdateMediumForMember)
	catGroup.POST("/tracks", auth.UnifiedAuthMiddleware(cfg, db), auth.RequireEmailVerified(db), catalogSvc.CreateTrackForMember)
	catGroup.PUT("/tracks/:id", auth.UnifiedAuthMiddleware(cfg, db), auth.RequireEmailVerified(db), catalogSvc.UpdateTrackForMember)

...
function HomeShowcaseContent() {
 ...
 <p className="font-mono text-sm text-gray-400">{t("shelf.emptyHint")}</p>
 <button onClick={() => setConfigOpen(true)} className="inline-flex items-center gap-2 px-3.5 h-9 max-sm:min-h-[44px] rounded-md bg-primary text-white text-sm font-semibold">
 <Settings2 className="w-4 h-4" /> {t("shelf.customize")}
 </button>
 </div>
 ) : (
 ...
 <div className={`w-8 h-8 rounded-sm grid place-items-center border ${getShelfColor(key, isCustom ? cs?.media_type : sys?.media_type)}`}>
 ...
 {w.original_title && (
 <p className="font-mono text-xs text-gray-500 line-clamp-1">
 {w.original_title}
 </p>
 )}
 </div>

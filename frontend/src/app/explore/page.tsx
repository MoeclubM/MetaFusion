    ...
    <div className="min-h-screen bg-background relative flex flex-col overflow-x-hidden selection:bg-primary selection:text-white">
      ...
      <main className="relative z-10 max-w-7xl mx-auto px-4 py-5 w-full flex-1 space-y-5">
        ...
        <div className="p-4 sm:p-6 rounded-lg border border-black/10 dark:border-white/[0.08] bg-surface/80 backdrop-blur-md shadow-soft space-y-3">
          ...
          {/* Type Tabs */}
          <div className="flex items-center gap-1.5 pt-2 border-t border-black/[0.06] dark:border-white/[0.06] overflow-x-auto no-scrollbar">
            <button
              onClick={() => handleSwitchType("works")}
              className={`inline-flex items-center gap-1.5 px-3 h-7.5 rounded-md text-xs font-semibold whitespace-nowrap border transition-all ${
                activeType === "works"
              ...
              {activeType === "works" && (
                <>
                  <button
                    ...
                    className={`h-7.5 px-2.5 rounded-md border text-xs font-mono flex items-center gap-1.5 transition-colors ${
                  ...
                  >
                    ...
                    {selectedTags.length > 0 && (
                      <span className="w-4 h-4 rounded-full bg-primary text-white text-[10px] grid place-items-center font-bold">
                        {selectedTags.length}
                      </span>
                    )}
                  </button>
                  <select
                    ...
                    className="h-7.5 px-2 rounded-md bg-black/[0.03] dark:bg-white/[0.04] border border-black/10 dark:border-white/10 text-xs text-gray-700 dark:text-gray-300 font-mono focus:outline-none"
                  >
                  ...
                  <div className="flex items-center gap-0.5 rounded-md border border-black/10 dark:border-white/10 bg-black/[0.03] dark:bg-white/[0.04] p-0.5">
                    <button
                      ...
                      className={`w-6.5 h-6.5 grid place-items-center rounded transition-colors ${
                    ...
                    >
                      <List className="w-3.5 h-3.5" strokeWidth={1.7} />
                    </button>
                  </div>
                </>
              )}
          ...
          {activeType === "works" && showTagFilterPanel && Object.keys(tagGroups).length > 0 && (
            <div className="p-3 rounded-lg bg-black/[0.02] dark:bg-white/[0.02] border border-black/5 dark:border-white/5 space-y-2.5 pt-2.5">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-2.5 text-xs">
                {Object.entries(tagGroups).map(([groupKey, tagsInGroup]) => (
                  <div key={groupKey} className="space-y-1 p-2 rounded-md bg-surface border border-black/5 dark:border-white/5">
                    <div className="font-mono text-[10px] uppercase text-gray-400 font-bold tracking-wider">
                      ...
                      {tagsInGroup.map((tg) => {
                        ...
                        return (
                          <button
                            ...
                            className={`px-1.5 py-0.5 rounded-sm text-[10px] font-mono transition-all flex items-center gap-1 border ${
                              isChecked
          ...
          {activeType === "artists" && (
            <div className="flex items-center gap-1 overflow-x-auto no-scrollbar pt-2 border-t border-black/[0.06] dark:border-white/[0.06]">
              <button
                onClick={() => {
                  setSelectedEntityType("");
                  updateUrl({ entity_type: "" });
                }}
                className={`px-2.5 h-6.5 rounded-md whitespace-nowrap border text-xs transition-all ${
                  selectedEntityType === ""
                    ? "bg-primary/10 text-primary border-primary/30 font-semibold"
          ...
          <div className="flex items-center justify-between font-mono text-xs text-gray-500 px-1 flex-wrap gap-2">
            <div className="flex items-center gap-1.5 flex-wrap">
              ...
              {selectedTags.map((tagName) => (
                <span
                  key={tagName}
                  className="px-2 py-0.5 rounded-md bg-emerald-500/10 text-emerald-500 border border-emerald-500/20 flex items-center gap-1 font-mono text-[11px]"
                >
                  <span>#{tagName}</span>
              ...
              {selectedEntityType && (
                <span className="px-2 py-0.5 rounded-md bg-sky-500/10 text-sky-500 border border-sky-500/20 flex items-center gap-1 font-mono text-[11px]">
              ...
              {queryParam && (
                <span className="px-2 py-0.5 rounded-md bg-amber-500/10 text-amber-500 border border-amber-500/20 font-mono text-[11px]">
        ...
        {activeType === "works" ? (
          loading ? (
            ...
            <div className="p-8 sm:p-10 rounded-lg border border-dashed border-black/10 dark:border-white/10 bg-surface/50 backdrop-blur-sm text-center space-y-3">
              ...
              <Link
                href="/contribute"
                className="inline-flex items-center gap-1.5 px-3.5 h-7.5 rounded-md bg-primary text-white keep-white font-semibold text-xs hover:opacity-90 transition-opacity shadow-xs"
              >
                ...
                >
                  ...
                  <div className="p-2.5 space-y-1 flex-1 flex flex-col justify-between">
                    <div>
                      ...
                      {w.original_title && <p className="font-mono text-[10px] text-gray-500 line-clamp-1">{w.original_title}</p>}
                    </div>
                    {w.tags && w.tags.length > 0 && (
                      <div className="flex flex-wrap gap-1 pt-0.5">
                        {w.tags.slice(0, 3).map((tg) => (
                          <span key={tg.id} className="px-1.5 py-0.2 rounded-sm bg-black/[0.04] dark:bg-white/[0.06] text-[9px] font-mono text-gray-600 dark:text-gray-400">
                            #{tg.name}
                          </span>
                        ))}
                        {w.tags.length > 3 && <span className="text-[9px] font-mono text-gray-400">+{w.tags.length - 3}</span>}
                      </div>
                    )}
                ...
                >
                  <div className="flex items-center gap-3 min-w-0">
                    ...
                    <div className="space-y-0.5 min-w-0">
                      <h3 className="font-semibold text-gray-900 dark:text-white text-xs truncate group-hover:text-primary transition-colors">
                        {w.title}
                      </h3>
                      ...
                      {w.tags && w.tags.length > 0 && (
                        <div className="flex flex-wrap gap-1 pt-0.5">
                          {w.tags.map((tg) => (
                            <span key={tg.id} className="px-1.5 py-0.2 rounded-sm bg-black/[0.04] dark:bg-white/[0.06] text-[9px] font-mono text-gray-500">
                              #{tg.name}
                            </span>
                          ))}
                        </div>
                  ...
                  <div className="text-primary text-xs font-mono flex items-center gap-1 shrink-0">
                ...
                >
                  <div className="flex items-start justify-between gap-2">
                    <div className="min-w-0">
                      <h3 className="font-semibold text-xs text-gray-900 dark:text-white truncate group-hover:text-primary transition-colors">
                        {a.name}
                      </h3>
                      {a.original_name && <p className="font-mono text-[10px] text-gray-500 truncate">{a.original_name}</p>}
                    </div>
                    <span className="shrink-0 px-1.5 py-0.2 rounded-sm bg-black/[0.04] dark:bg-white/[0.06] border border-black/5 dark:border-white/5 text-[9px] font-mono text-gray-600 dark:text-gray-400">
                      {t(`entity.${a.entity_type}`) !== `entity.${a.entity_type}` ? t(`entity.${a.entity_type}`) : a.entity_type}
                    </span>
                  </div>
                  {a.disambiguation && <p className="text-[11px] text-gray-500 line-clamp-2">{a.disambiguation}</p>}
                  {a.biography && <p className="text-[11px] text-gray-600 dark:text-gray-400 line-clamp-2">{a.biography}</p>}
                  <div className="flex items-center gap-2 font-mono text-[10px] text-gray-500 pt-1.5 border-t border-black/[0.04] dark:border-white/[0.04]">
              ...
              >
                <div className="min-w-0 space-y-0.5">
                  ...
                  <div className="flex items-center gap-1.5 pt-0.5">
                    ...
                    {r.packaging && <span className="text-[10px] font-mono text-gray-500">{r.packaging}</span>}
                  </div>
                </div>
                <div className="text-primary text-xs font-mono flex items-center gap-0.5 shrink-0">
                  <span>{t("explore.detail")}</span>
                  <ArrowRight className="w-3 h-3" />
                </div>
              </Link>

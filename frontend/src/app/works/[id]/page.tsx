          ...
          <div className="flex flex-col sm:flex-row gap-4 sm:gap-5">
            <div className="w-32 sm:w-40 shrink-0">
              <div className="aspect-[3/4] rounded-md overflow-hidden border border-black/10 dark:border-white/10 bg-background shadow-xs">
                {work.cover_image_url ? (
                  ...
                  <div className="w-full h-full grid place-items-center font-display text-gray-400 p-3 text-center text-xs">{work.title}</div>
                )}
              </div>
              <div className="mt-2 flex items-center gap-1.5 font-mono text-[10px] text-gray-500">
                <Star className="w-3 h-3 text-amber-500" strokeWidth={1.5} /> {t("work.detail.viewCount", { count: work.view_count })}
            ...
            <div className="flex-1 space-y-3 min-w-0">
              <div className="flex flex-wrap items-center gap-1.5 font-mono text-[10px] tracking-wide">
                <span className="px-2 py-0.5 rounded-sm bg-primary text-white font-semibold">{t("work.detail.workBadge")}</span>
                <span className="px-2 py-0.5 rounded-sm bg-black/[0.04] dark:bg-white/[0.06] border border-black/10 dark:border-white/10 text-gray-700 dark:text-gray-300">{work.category ? categoryDisplayName(work.category as Category, locale) : work.media_type}</span>
                <TemporalBadge
              ...
              <div>
                ...
                {work.original_title && <p className="font-mono text-xs text-gray-500 dark:text-gray-400 mt-0.5">{work.original_title}</p>}
                {work.aliases && work.aliases.length > 0 && <p className="font-mono text-[10px] text-gray-400 dark:text-gray-500 mt-0.5">{t("work.detail.alias", { value: work.aliases.join(" / ") })}</p>}
              </div>
                ...
                <div className="flex flex-wrap gap-1">
                  {work.artist_relations.map((rel) => (
                    <Link
                      ...
                      className="inline-flex items-center gap-1 px-2 py-0.5 rounded-sm bg-black/[0.03] dark:bg-white/[0.04] border border-black/10 dark:border-white/10 hover:border-primary/40 text-xs text-gray-700 dark:text-gray-200 transition-colors"
                    >
                      <User className="w-3 h-3 text-primary" strokeWidth={1.5} />
                      <span className="font-mono text-[10px] text-gray-500">{getRoleName(rel.role, t)}:</span>
                      <span className="underline decoration-dotted underline-offset-2">{rel.artist?.name}</span>
                    </Link>
                  ))}
                </div>
              )}
              {work.summary && <p className="text-xs leading-relaxed text-gray-600 dark:text-gray-400 max-w-3xl line-clamp-3">{work.summary}</p>}
              {work.tags && work.tags.length > 0 && (
                <div className="flex flex-wrap gap-1 pt-0.5">
                  {work.tags.map((tag) => (
                    <span key={tag.id} className="px-1.5 py-0.2 rounded-sm bg-black/[0.04] dark:bg-white/[0.04] border border-black/5 dark:border-white/10 font-mono text-[10px] text-gray-600 dark:text-gray-400">
                      #{tag.name}
                    </span>
                  ))}
                </div>
        ...
        <section className="rounded-lg border border-black/10 dark:border-white/[0.08] bg-surface/80 backdrop-blur-md shadow-soft overflow-hidden">
          <div className="px-3.5 sm:px-4 py-3 border-b border-black/5 dark:border-white/[0.06] flex flex-col sm:flex-row sm:items-center justify-between gap-2.5">
            <div className="flex items-center gap-2">
              ...
              <span className="font-mono text-[11px] text-gray-500">{t("work.detail.totalReleases", { count: total })}</span>
            </div>
            <div className="flex items-center gap-2">
              <form onSubmit={onSearch} className="relative w-full sm:w-auto">
                ...
                <input
                  ...
                  className="pl-8 pr-2.5 h-7.5 w-full sm:w-48 bg-black/[0.03] dark:bg-white/[0.04] border border-black/10 dark:border-white/10 rounded-md text-xs text-gray-900 dark:text-white placeholder:text-gray-400 focus:outline-none focus:border-primary/50 font-mono"
                />
              </form>
              {user && (
                <button onClick={() => setIsUploaderOpen(true)} className="inline-flex items-center gap-1 h-7.5 px-3 rounded-md bg-primary text-white text-xs font-semibold hover:opacity-90 transition-opacity">
                  <UploadCloud className="w-3.5 h-3.5" strokeWidth={1.6} />
                    ...
                    {releases.map((rel) => (
                      <tr key={rel.id} className="hover:bg-black/[0.02] dark:hover:bg-white/[0.02] transition-colors">
                        <td className="py-2.5 px-3.5">
                          <Link href={`/releases/${rel.id}`} className="font-semibold text-gray-900 dark:text-white hover:text-primary inline-flex items-center gap-1">
                            {rel.edition_name} <ArrowUpRight className="w-3 h-3 text-gray-400" strokeWidth={1.6} />
                          </Link>
                          {rel.uploader && <span className="ml-2 font-mono text-[10px] text-gray-500">by {rel.uploader.username}</span>}
                        </td>
                  ...
                  <a key={rel.id} href={`/releases/${rel.id}`} className="block px-3.5 py-3 active:bg-black/[0.02] dark:active:bg-white/[0.04]">
                    <div className="flex items-start justify-between gap-3">
                      <div className="min-w-0 space-y-0.5">
                        <div className="font-semibold text-gray-900 dark:text-white text-xs leading-tight line-clamp-2 inline-flex items-center gap-1">{rel.edition_name} <ArrowUpRight className="w-3 h-3 text-gray-400 shrink-0" strokeWidth={1.6} /></div>
                        <div className="font-mono text-[10px] text-gray-500 truncate">{rel.publisher_entity ? rel.publisher_entity.name : rel.publisher || "—"} {rel.catalog_number ? "· " + rel.catalog_number : ""}</div>
                        <div className="font-mono text-[10px] text-gray-400">{rel.edition_date ? new Date(rel.edition_date).toLocaleDateString() : "—"}{rel.uploader ? " · by " + rel.uploader.username : ""}</div>
                      </div>
              ...
              <div className="px-3.5 py-2.5 border-t border-black/5 dark:border-white/[0.06] bg-black/[0.01] dark:bg-white/[0.01] flex items-center justify-between">
                ...
                <div className="flex items-center gap-1.5">
                  <button
                    ...
                    className="w-7 h-7 grid place-items-center rounded-md bg-black/[0.03] dark:bg-white/[0.06] border border-black/10 dark:border-white/10 text-gray-600 dark:text-gray-300 disabled:opacity-40 hover:bg-black/[0.06] dark:hover:bg-white/[0.10]"
                  >
                  ...
                  <Link href={`/works/${workId}/releases`} className="ml-1 inline-flex items-center gap-1 h-7 px-2.5 rounded-md bg-black/[0.03] dark:bg-white/[0.06] border border-black/10 dark:border-white/10 text-xs font-mono text-gray-700 dark:text-gray-300 hover:text-primary transition-colors">
                    {t("work.detail.viewAll")} <ArrowRight className="w-3 h-3" strokeWidth={1.6} />
                  </Link>
                </div>
              </div>
            </>
        ...
        <section className="p-4 sm:p-6 rounded-lg border border-black/10 dark:border-white/[0.08] bg-surface/80 backdrop-blur-md shadow-soft">
          <div className="flex items-center justify-between border-b border-black/5 dark:border-white/[0.06] pb-2">
            <h3 className="font-display text-sm font-bold text-gray-900 dark:text-white flex items-center gap-1.5">
              ...
              <span className="font-mono text-xs font-normal text-gray-500">({topics.length})</span>
            </h3>
            <Link href="/community?board_code=comment" className="font-mono text-xs text-primary hover:underline inline-flex items-center gap-0.5">
              <span>{t("work.detail.enterForum")}</span>
              <ArrowRight className="w-3 h-3" strokeWidth={1.5} />
            </Link>
          </div>
          {topics.length === 0 ? (
            <p className="font-mono text-xs text-gray-500 mt-2">{t("work.detail.noRelatedTopics")}</p>
          ) : (
            <div className="divide-y divide-black/5 dark:divide-white/[0.06] mt-2">
              {topics.slice(0, 3).map((t) => (
                <Link key={t.id} href={`/community/${t.id}`} className="py-2.5 flex items-center justify-between hover:bg-black/[0.02] dark:hover:bg-white/[0.02] px-2 rounded-md transition-colors">
                  <span className="text-xs text-gray-800 dark:text-gray-200 truncate pr-4">{t.title}</span>
                  <span className="font-mono text-[10px] text-gray-500 shrink-0">{new Date(t.created_at).toLocaleDateString()}</span>
                </Link>
              ))}
            </div>
          )}

          ...
          {loading ? (
            ...
            <>
              ...
              <div className="sm:hidden divide-y divide-white/[0.06]">
                {releases.map((rel)=>(<a key={rel.id} href={`/releases/${rel.id}`} className="block px-4 py-3.5 active:bg-white/[0.04]"><div className="flex items-start justify-between gap-3"><div className="min-w-0 space-y-1"><div className="font-semibold text-white text-sm line-clamp-2 inline-flex items-center gap-1">{rel.edition_name} <ArrowUpRight className="w-3 h-3 text-gray-500 shrink-0" strokeWidth={1.5} /></div><div className="font-mono text-[11px] text-gray-400 truncate">{rel.publisher || "—"} {rel.catalog_number ? "· " + rel.catalog_number : ""}</div><div className="font-mono text-[11px] text-gray-500">{rel.edition_date ? new Date(rel.edition_date).toLocaleDateString() : "—"}</div></div><span className={`shrink-0 px-2 py-1 rounded-full text-[11px] font-medium border ${rel.is_master_verified ? "bg-emerald-500/10 border-emerald-500/20 text-emerald-300" : "bg-white/[0.04] border-white/10 text-gray-400"}`}>{rel.is_master_verified ? t("work.detail.verified") : t("work.detail.pending")}</span></div></a>))}
              </div>

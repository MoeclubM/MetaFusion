...
import { getAuthLoginUrl, AUTH_SERVICE_URL } from "@/lib/services";
...
function LoginInner() {
 ...
 // 注册流程尚未在后端实现，若访问方显式请求注册 tab，则如实提示不可用。
 const [notice, setNotice] = useState<string | null>(tabParam === "register" ? t("auth.registrationClosed") : null);
 ...
 // 注册入口：仅当实例尚未初始化时指向 /setup（首次管理员初始化）；
 // 否则如实提示注册未开放，不再向 /auth/register 这类不存在的接口发请求。
 ...
 {AUTH_SERVICE_URL.startsWith("http") && (
   <a
     href={getAuthLoginUrl(searchParams.get("redirect") || "/")}
 ...
 {notice && (
 <div className="mx-4 sm:mx-5 mt-3.5 p-3.5 rounded-lg bg-amber-500/10 border border-amber-500/20 text-amber-600 dark:text-amber-300 font-mono text-sm flex items-start gap-2">
 ...
 <>
 <span>{t("auth.gate.secureLogin")}</span>
 <ArrowRight className="w-4 h-4" />
 </>
 )}
 </button>

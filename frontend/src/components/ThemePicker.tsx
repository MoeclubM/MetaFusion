  ...
  return (
    <div className="relative" ref={containerRef}>
      <button
        ...
        className="w-7.5 h-7.5 grid place-items-center rounded-md bg-black/5 dark:bg-white/[0.04] hover:bg-black/10 dark:hover:bg-white/[0.08] border border-black/10 dark:border-white/10 text-gray-700 dark:text-gray-300 transition-colors"
      >
        {resolvedMode === "dark" ? (
          <Moon className="w-3.5 h-3.5 text-sky-400" strokeWidth={1.7} />
        ) : (
          <Sun className="w-3.5 h-3.5 text-amber-500" strokeWidth={1.7} />
        )}
      </button>
      ...
      {isOpen && (
        <div
          ...
          className="absolute right-0 mt-1.5 w-60 rounded-lg border border-black/10 dark:border-white/[0.08] bg-surface shadow-elevated p-3 z-50 animate-slide-up text-xs space-y-3"
        >
          {/* Header */}
          <div className="flex items-center justify-between border-b border-black/[0.06] dark:border-white/[0.06] pb-2 font-mono text-[10px] text-gray-500 dark:text-gray-400">
          ...
          <div className="space-y-1">
            <div className="text-[10px] font-mono text-gray-500 dark:text-gray-400">{t("theme.displayMode")}</div>
          ...
          <div className="space-y-1 border-t border-black/[0.06] dark:border-white/[0.06] pt-2">
            <div className="flex items-center justify-between text-[10px] font-mono text-gray-500 dark:text-gray-400">
              ...
              <span className="font-semibold text-gray-900 dark:text-white text-[10px]">
              ...
              {accents.map((item) => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      {isSelected && <Check className="w-3 h-3 text-white keep-white stroke-[2.5]" />}
                    </div>
                  </button>
                );
              })}

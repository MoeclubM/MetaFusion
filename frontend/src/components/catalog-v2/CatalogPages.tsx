    ...
    <>
      ...
      <div className="cv-table-scroll">
        <table>
          <thead>
            <tr>
              <th>{t("catalogV2.attributes")}</th>
              {items.map((x) => (
                <th key={x.release.id}>
                  <Link href={`/catalog/${x.release.id}`}>
          ...
          <tbody>
            ...
            <tr>
              ...
              {items.map((x, index) => (
                <td key={x.release.id}>
                  {x.media.map((m: any) => (
                    <section key={m.medium.id}>
                      <h3>{title(m.medium, locale)}</h3>
                      <FieldValue

...
- 搜索（`/api/catalog/entities?q=...` 列表与详情，基于 OpenSearch 2.x 构建）、社区帖子列表与详情的文字部分
- 首页、探索页、榜单等聚合页
...
- 任何媒体资产二进制（CAS / 资产登记）：`GET /api/archive/resources/:id/content` 预签名下载链接、`masters/*` 原档、`previews/*` HLS 切片（`index.m3u8` / `segment_*.ts`）、音频 `preview.m4a`、图像 `preview.webp`、波形/字幕/CUE/Log 附属文件
- 上传链路：`POST /api/archive/entities/:id/resources`（`POST /api/catalog/entities` 创建实体）
- 社区写入：发帖、回帖、评注
- 个人数据：邀请信息、已邀请用户列表
...
- **可开关**：`system_settings.registration_enabled`（总闸）与 `invite_required`（是否强制邀请）由 `admin`/`archivist` 在后台 `系统设置` 中动态切换；注册与登录开关由 `/api/setup`、`/api/auth/*` 暴露，具体字段以该端点实际响应为准。
...
| AUTH-04 | 邀请链 | 注册成功写入 `users.invited_by`，`InviteCode` 为 `MF-` 永久码（邀请相关端点当前实现缺失，见顶部过期说明） |
...
| ID | 需求 | 说明 |
|---|---|---|
| MEDIA-01 | 下载需登录 | `GET /api/archive/resources/:id/content` 强制认证，返回预签名 URL，`response-content-disposition` 带文件名（需启用 archive 模块） |
...
- **审计**：所有 `L1` 写入与媒体资产（CAS / `modules.resources`）访问经 `admin_audit_logs` 记录 `actor/target/ip/ua`。
...
- **后端**：`catalog` 读接口公开；可选 `archive` / `playback` 模块的媒体内容接口强制登录；检索走 `catalog/entities` 的 SQL 全文路径，OpenSearch 索引由 worker 维护。
- **网关**：`deploy/nginx.conf` 对媒体预览路径的反代需透传并校验 `Authorization`，RustFS 预览桶不得设为 `public`。

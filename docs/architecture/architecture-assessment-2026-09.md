...
- 固定八实体 kind：`agent / collection / work / content_unit / expression / release / medium / track`（`backend/internal/catalog/types.go` 的 `Kinds` 常量，数据库 CHECK 见 `backend/migrations/000001_catalog_core.up.sql`）。
- 动态类型/字段/词表/关系/模板由 `catalog.definitions.document`（JSON）驱动（见 `backend/migrations/000001_catalog_core.up.sql` 的 `catalog.definitions`），后台 DefinitionsEditor 走 draft → impact → publish（`backend/internal/catalog/definitions.go` 的 `Draft` / `Impact` / `Publish`），代码默认值见 `backend/internal/catalog/defaults.go`。
...
| 需求 | 结论 | 证据与说明 |
| --- | --- | --- |
| (a) 同一专辑多版本（普通版/限定版/地区版，各自曲目与内含特典不同） | **已支持** | 一个专辑 Work 下建多个 Release，版本类型由 `edition_type` 词表承载（`defaults.go` 的字段声明与词表种子，接入 release 的字段集由 `structure` 与模板声明，枚举校验走 `validation.go` 的 `value`/`attributes`）；各版本曲目差异由各自 Medium/Track 与 `track_contents` 独立表达（库结构见 `backend/migrations/000001_catalog_core.up.sql`，写入见 `store.go` 的 `Save`：`track_contents` 整组替换）；特典盘用 `medium.role=supplement`（`defaults.go` 的 `role` 词表项）；盒内附件 `attachments`、店铺特典 `store_bonuses`（`defaults.go` 的字段与其 `items` 子字段结构）；多版本结构对比见 `relations.go` 的 `Compare`（仅比较 `Comparable` 字段与完整 Release 树）。 |
...
1. **多作品盒装缺显式汇编语义**
   - 现状：跨作品收录只能靠 `release_subjects` 多行声明（库结构见 `backend/migrations/000001_catalog_core.up.sql`，role 取自 `defaults.go` 的 `release_role` 词表：`primary/compilation/supplement`）加 `includes` 关系（`defaults.go` 的 `addRel("includes", …)`，collection/work → work/collection）。没有专门的 compilation/boxset 实体或 release 级汇编模型；文档要求“独立汇编 Work”，但 `TrackContent` 强制被收录 Expression 的 Work 必须列入 release subjects（`store.go` 的 `Save` 归属校验），导致汇编 Work 与母作品语义重叠。
...
2. **跨 Work 一致性只在应用层、且是全局 EXISTS 扫描**
   - 现状：`Save` 每次写入都执行一次全表 EXISTS 检查，只要库内任意 `track_contents` 的 Expression 所属 Work 未列入对应 Release subjects 就整笔失败（`store.go` 的归属校验），错误码 `undeclared_release_subject`。数据库层没有等价约束（`track_contents` 仅 FK 到 tracks/expressions，见 `backend/migrations/000001_catalog_core.up.sql`）。
...
以上代码为唯一事实来源（`backend/internal/catalog/defaults.go` 的关系种子），便于文档比对：
...
- 关系通用字段（所有种子关系共用）：`role`、`credit_role`、`character_rank`、`context`、`character`、`language`、`begin_date`、`end_date`、`scope`（`defaults.go` 的 `addRel`）。
...
- 详情页属性分区与头部徽章：按模板 `sections` / `badge_fields` 递归渲染（`TemplateAttributeSections.tsx`），未覆盖字段自动收尾；字段类型闭集为 text / multilingual / number / date / boolean / url / enum / entity / list / group（以 `validation.go` 的 `validateField` 为准）。

...
- `release` 补 `edition_type` 枚举字段，词表 `edition_type`：`standard` 普通版、`limited` 限定版、`first_press` 初回版、`regional` 地区版、`reissue` 再版、`digital` 数字版；并把既有的 `country / platform / publisher / edition_date / catalog_number / barcode / attachments / store_bonuses / events` 列为发行页必展字段（发行版的语言走实体级 `original_language` / `translations`，`release` 类型未声明 `language` 字段）。

...
- 本地 Agent 可直接读写仓库内已忽略的专用凭据文件（如 `../metafusion-skills/skills/metafusion-curator/local/credentials.json`）并只向目标实例使用；不读不输出无关的 `.env`、密钥、PAT、数据库导出。示例用占位符，真实凭证不进日志、文档或提交。
